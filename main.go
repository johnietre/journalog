...
func (s *Server) NewUserHandler(w http.ResponseWriter, r *http.Request) {
  ...
  if err := json.NewDecoder(r.Body).Decode(&user); err != nil {
    // TODO: Handle errors
    newRespErr(statusBR, "invalid user object").WriteTo(w)
    return
  } else if !checkEmail(user.Email) {
    newRespErr(statusBR, "invalid credentials").WriteTo(w)
    return
  }
  hash, err := hashPassword(password)
  if err != nil {
    newRespErr(statusBR, "bad credentials").WriteTo(w)
    return
  }
  user.passwordHash, user.CreatedAt = hash, time.Now().Unix()
  if err := s.newUser(&user); err != nil {
    if errors.Is(err, errUserExists) {
      newRespErr(statusBR, "user with email already exists").WriteTo(w)
    } else {
      newRespErr(statusISE, iseText).WriteTo(w)
      logpkg.Printf("error creating user for email %s: %v", user.Email, err)
    }
  } else if false {
    // TODO: create journals directory
  } else {
    newRespOk(user).WriteTo(w)
  }
}
...
func (s *Server) deleteUser(id uint64) error {
  ...
  // NOTE: No need to delete logs explicitly since they should cascade
  rows, err := s.db.Query(`SELECT path FROM journals WHERE user_id=?`, id)
  if err != nil {
    return err
  }
  defer rows.Close()
  ...
  _, err = s.db.Exec(`DELETE FROM journals WHERE user_id=?`, id)
  return err
}
